...
async function userController(ws, socket, message) {
    ...
    switch(body.type) {
        case SIGNIN: 
            ...
            await user.save();
            return socket.send(JSON.stringify({code: 400, status: 'error', message: 'user created'}));
            break;

        case LOGIN: 
            const userLogin = await User.findOne({"email": body.data.email});
            console.log(userLogin);
            if (!userLogin) return socket.send(JSON.stringify({code: 400, status: 'error', message: 'Sorry account doesnt exit'}));

            if (userLogin.password !== body.data.password) {
                return socket.send(JSON.stringify({code: 400, status: 'error', message: 'Sorry wrong credentials'}));
            }
